...
class CertificateService
{
    ...
    public function generatePDF($certificados, string $tipo, $empresa): array
    {
        ...
        Log::info("Tipo: {$tipo}");
        Log::info("Empresa data: " . json_encode($empresa));
     ...
     * Generar nombre de archivo
     */
    public function generateFileName($constructor, string $tipo, string $serial): string
    {
        $fecha = now()->format('Y-m-d');
        $nit = $constructor->nitempresa ?? 'SIN_NIT';
        $fileName = "{$serial}.pdf";
    ...
    private function obtenerEstadisticasMensuales(?string $nit): array
    {
        ...
        for ($i = 5; $i >= 0; $i--) {
            ...
            $inicioMes = $fecha->startOfMonth()->toDateTimeString();
            $finMes = $fecha->endOfMonth()->toDateTimeString();
